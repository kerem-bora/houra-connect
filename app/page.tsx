...
// --- SUB-COMPONENTS (Dışarı alındı ki hata vermesin) ---
...
export default function Home() {
  ...
  const [offerResults, setOfferResults] = useState<any[]>([]); // null yerine boş dizi yapıldı (çökme koruması)
  ...
  const handleAddNeed = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/needs", {
        ...
        body: JSON.stringify({
          fid: context.user.fid, username: context.user.username, location: needLocation || "Global",
          text: needText, wallet_address: currentAddress.toLowerCase(), price: needPrice.toString(),
        }),
      });
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/profile", { 
        ...
        headers: { "Content-Type": "application/json", "x-farcaster-fid": context.user.fid.toString() }, 
        body: JSON.stringify({ 
          fid: Number(context.user.fid), username: context.user.username, 
          city: location, talents: offer, address: currentAddress.toLowerCase() 
        }) 
      });
      if (res.ok) { setStatus("Profile Updated! ✅"); setTimeout(() => setStatus(""), 2000); }
      else { const errData = await res.json(); setStatus(`Error: ${errData.error || "Failed"}`); }
  ...
  const handleDeleteNeed = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setNeeds(prev => prev.filter(n => n.id !== id)); setStatus("Deleted! ✅"); setTimeout(() => setStatus(""), 2000); }
      else { const err = await res.json(); setStatus(err.error || "Delete failed"); }
  ...
  const AboutContent = () => (
    <div style={{ background: '#111', border: '1px solid #333', borderRadius: '24px', padding: '25px', maxWidth: '400px', width: '100%', position: 'relative', textAlign: 'left' }}>
      ...
      {!isFarcaster && (
        <>
          ...
          <div style={{ margin: '20px 0', fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <p>1. Download the <a href="https://join.base.app/" target="_blank" rel="noopener noreferrer" style={{ color: '#fff', fontWeight: 'bold', textDecoration: 'underline' }}>Base app</a>.</p>
            <p>2. Create your wallet. 3. Search for <strong>"Houra"</strong>.</p>
          </div>
        </>
      )}
      ...
      {isFarcaster && (
        <button onClick={() => setIsAboutOpen(false)} style={{ width: '100%', padding: '12px', background: '#fff', color: '#000', border: 'none', borderRadius: '12px', fontWeight: 'bold', marginTop: '15px' }}>
          Got it!
        </button>
      )}
    </div>
  ...
  const randomOffers = offerResults.length > 0 ? [...offerResults].sort(() => 0.5 - Math.random()).slice(0, 10) : [];
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', padding: '20px', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '5px' }}>
        ...
        <button onClick={() => setIsAboutOpen(true)} style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', borderRadius: '50%', width: '32px', height: '32px', fontSize: '1.1rem' }}>i</button>
      </div>
      ...
      {/* 1. SEND PANEL */}
      <div style={{ padding: '20px', borderRadius: '24px', background: 'linear-gradient(135deg, #2563eb 0%, #7e22ce 100%)', marginBottom: '20px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <input placeholder="Search offer, location..." value={offerQuery} onChange={(e) => setOfferQuery(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '12px', background: '#111', border: '1px solid #333', color: '#fff', boxSizing: 'border-box' }} />
            ...
            {offerResults.map(user => (
              <div key={user.fid} style={{ padding: '12px', background: '#111', borderRadius: '12px', border: '1px solid #222', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <p style={{ margin: 0, fontSize: '0.75rem', color: '#666' }}>📍 {user.city || "Global"} • {user.talents || "No offer"}</p>
                </div>
      ...
      <details style={{ background: '#111', padding: '12px', borderRadius: '15px', marginBottom: '20px', border: '1px solid #222' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '12px' }}>
          ...
          <button onClick={handleSaveProfile} disabled={!currentAddress} style={{ padding: '12px', background: currentAddress ? '#333' : '#550000', color: '#fff', fontWeight: 'bold', borderRadius: '10px', border: 'none' }}>
          ...
          <div key={idx} style={{ padding: '16px', background: '#111', borderRadius: '20px', border: '1px solid #222' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              ...
              {context?.user?.fid && Number(need.fid) === Number(context.user.fid) && (
                <button onClick={() => handleDeleteNeed(need.id)} style={{ background: 'none', border: 'none', color: '#ff4444', fontSize: '0.7rem', textDecoration: 'underline' }}>Delete</button>
              )}
            </div>
            <p style={{ margin: '0 0 10px 0', fontSize: '0.9rem', color: '#ccc' }}>{need.text}</p>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
               <span style={{ fontSize: '0.7rem', color: '#666' }}>📍 {need.location} • <span style={{ color: '#2563eb', fontWeight: 'bold' }}>⏳ {need.price} Houra</span></span>
               <button onClick={() => sdk.actions.viewProfile({ fid: Number(need.fid) })} style={{ color: '#2563eb', background: 'none', border: 'none', fontWeight: 'bold', fontSize: '0.75rem' }}>VIEW</button>
            </div>
          </div>
        ))}
      </div>

      {/* MODALS (Orijinal içerikler korundu) */}
      {activeModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.9)', backdropFilter: 'blur(10px)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#111', border: '1px solid #333', borderRadius: '24px', padding: '25px', width: '100%', maxWidth: '450px', maxHeight: '80vh', overflowY: 'auto', position: 'relative' }}>
            ...
            {activeModal === 'needs' && (
              <div>
                <h3 style={{ marginTop: 0, color: '#2563eb' }}>Latest Needs</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '15px' }}>
                  {needs.map((n, i) => (
                    <div key={i} style={{ padding: '12px', background: '#000', borderRadius: '12px', border: '1px solid #222' }}>
                      <div style={{ fontWeight: 'bold', fontSize: '0.8rem' }}>@{n.username}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '15px' }}>
                  {randomOffers.map((user: any, i: number) => (
                    <div key={i} style={{ padding: '12px', background: '#000', borderRadius: '12px', border: '1px solid #222' }}>
                      ...
                      <div style={{ fontSize: '0.85rem', color: '#ccc' }}>{user.talents}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {activeModal === 'active' && (
              <div><h3 style={{ marginTop: 0, color: '#2563eb' }}>Active Members</h3><p style={{ fontSize: '0.85rem', color: '#ccc' }}>Members with most Houra exchange.</p></div>
            )}
          </div>
        </div>
      )}
